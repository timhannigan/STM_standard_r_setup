...
# note: this is important to ensure that we're using the latest version of 
# LDAVis, with the working version of topic order
...
Some basic pre-processing has been done already using the Python Jupyter notebook. We'll attempt to use Quanteda and the standard STM functions to ensure that we can effectively use the covariates in the model.
...
data <- dfm(data, stem = FALSE, remove = stopwords('english'),
...
kResult <- searchK(out$documents, out$vocab, K = K_, init.type = "Spectral", data = out$meta, verbose=FALSE)
end.time <- Sys.time()
...
This list of covariates should match the searchK formula above, i.e. prevalence =\~ months_since_origin + political_leaning

```{r}
i <- 25 # this is where we set the topic specification
set.seed(02138)
# Specification for K topics using function stm() 
model <- stm(out$documents, out$vocab, K = i, max.em.its = 150, data = out$meta, init.type = "Spectral", verbose=FALSE)
...
topics <- data.frame("features" = t(topics$frex)) # using FREX
...
```{r}
set.seed(02138)
#covariates_est <- estimateEffect(~ months_since_origin + political_leaning, model, uncertainty = "None", metadata = out$meta) 
```
...
pdf(sprintf("artifacts_for_rendering/topic_%s_sample_docs.pdf", j), width=10, height=8.5)
...
pdf(sprintf("artifacts_for_rendering/topic_%s_word_cloud.pdf", j), width=5, height=5)
cloud(model, topic=j, scale=c(2,.25))
dev.off()
...
for (j in 1:i){
  ...
  thoughts_topic<- findThoughts(model, texts = out$meta$documents,
                                n = 4, topics = j)
  pdf(sprintf("artifacts_for_rendering/topic_artifacts/topic_%s_sample_docs.pdf", j), width=10, height=8.5)
  plotQuote(thoughts_topic, width = 225, text.cex = .5, maxwidth = 1500)
  dev.off()
  
  # project the words in a topic
  set.seed(02138)
  pdf(sprintf("artifacts_for_rendering/topic_artifacts/topic_%s_word_cloud.pdf", j), width=5, height=5)
  cloud(model, topic=j, scale=c(2,.25))
  dev.off()
...
###
j <- 2
pdf(sprintf("artifacts_for_rendering/topic_artifacts/topic_%s_by_Y", j), width=10, height=8.5)
